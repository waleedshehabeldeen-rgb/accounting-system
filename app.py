...
# Aiven MySQL Config
MYSQL_CONFIG = {
    "host":     "mysql-c7a6ae0-cuc-2be0.k.aivencloud.com",
    "port":     26661,
    "user":     "avnadmin",
    "password": "AVNS_SKOI2C-x_wvMMg7KZVD",
    "database": "defaultdb",
    "charset":  "utf8mb4",
    "ssl_ca":   "/mount/src/accounting-system/ca.pem",
...
if page == "🏠  الرئيسية":
    ...
    if selected_code:
            ...
            conn = get_conn()
            j_query = "SELECT * FROM journal_lines WHERE account_code=%s AND company_id=%s"
            j_params = [code, co_id]
            if date_from_led:
                j_query += " AND entry_date >= %s"; j_params.append(date_from_led)
            if date_to_led:
                j_query += " AND entry_date <= %s"; j_params.append(date_to_led)
            j_query += " ORDER BY id"
